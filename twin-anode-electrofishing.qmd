---
# Document details
title: "Considerations when deciding whether to do depletion electrofishing with single or twin anodes"
...
**Short title** Population estimates from single and twin anode electrofishing can differ
...
Although the theory for population estimation by depletion electrofishing applies equally to sampling by single and twin anodes, changing from single- to twin-anode sampling (or vice versa), due to changes in river channel (e.g., due to installation or removal of an impoundment) is not straightforward.
...
4.    Care should be taken when deciding to sample by depletion electrofishing using a single or twin anode sampling strategy. When the assumptions of the sampling strategies are identical and met, then estimators will give the same population estimates. If, however, the assumptions of the sampling strategies are different or not met, then the estimates will be different and incomparable (without suitable correction).
...
# Introduction (600)
  ...
  - We assume e-fishing surveyor biases their effort toward suitable habitat
  - We note that twin versus single anode surveying will (at least?) double the area exposed to the electric field and so should therefore increase catch
...
## Plan
  ...
  1.  Create a closed population distributed throughout the survey area in accordance with some habitat suitability measure, including flow, depth, and Ranunculus cover
  2.  Sample the population as a correlated random walk (CRW) influenced by the habitat suitability measure and using a single anode with effective detection radius (EDR) specified as a function of distance and conductivity
  3.  Repeat the sampling with twin anodes ensuring no overlap between their EDRs
  4.  Optionally, allow for overlap between twin anode EDRs
