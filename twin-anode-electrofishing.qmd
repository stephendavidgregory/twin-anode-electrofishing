...
1.    Simulate example in which the population estimates are the same whether using single or twin anode sampling strategies, i.e., where *p* using single anode is half of *p* using twin anode.

2.    Simulate example in which the population estimates are different when using single or twin anode sampling strategies because a certain proportion of the fish population circumvent the anode and are thereby unavailable to the single anode sampling strategy. E.g., if number of fish caught differ, but $p$ is estimated to be similar for single- and twin- anode fishing, then the total estimates will be proportionally different and suggest that a proportion of the population is not available to be captured by single-anode fishing.
